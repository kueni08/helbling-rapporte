...
function confirmImport(importId, allowDuplicate = false) {
  ...
  const duplicate = db.prepare(`SELECT li.*, o.order_number FROM lieferschein_imports li
    ...
    (li.file_hash=? OR (? IS NOT NULL AND li.lieferschein_nr=?) OR (? IS NOT NULL AND li.projekt_nr=?)) LIMIT 1`)
    .get(importId, imp.file_hash, imp.lieferschein_nr, imp.lieferschein_nr, imp.projekt_nr, imp.projekt_nr);
