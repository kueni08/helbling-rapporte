...
function confirmImport(importId, allowDuplicate = false) {
  const db = getDb();
  ...
  const data = JSON.parse(imp.raw_json || '{}');
  const duplicate = db.prepare(`SELECT li.*, o.order_number FROM lieferschein_imports li
    ...
    .get(importId, imp.file_hash, imp.lieferschein_nr, imp.lieferschein_nr, imp.projekt_nr, imp.projekt_nr, imp.kunde, imp.lieferdatum);
...
module.exports = { startWatcher, stopWatcher, getInboxDir, processFile, confirmImport, DEFAULT_EXTRACTION_PROMPT };
