...
async function extractLieferscheinData(pdfText) {
  ...
  const prompt = `Du analysierst einen Schweizer Lieferschein (Helbling & Co. AG, SIBOX/FacetteStar Produkte).
...
Erwartetes JSON-Format:
{
  ...
  "artikel": [
    {
      "artikel_nr": "Z1500000",
      "beschreibung": "Panzerverschluss SIBOX 105/Z15, kpL.",
      "menge": 2.0,
      "einheit": "Stk."
    }
  ]
...
function upsertArticles(db, artikelList) {
  ...
  for (const a of artikelList) {
    ...
    if (existing) {
      db.prepare(`UPDATE articles SET name = ?, unit = ?, updated_at = datetime('now') WHERE id = ?`)
        .run(a.beschreibung || a.artikel_nr, a.einheit || 'Stk.', existing.id);
    } else {
...
function createOrder(db, data, importId) {
  ...
  const kundeStr = data.kunde
    ? [data.kunde.name, data.kunde.strasse, `${data.kunde.plz} ${data.kunde.ort}`.trim()].filter(Boolean).join(', ')
    : null;

  const adresseStr = data.kunde
    ? [data.kunde.strasse, `${data.kunde.plz} ${data.kunde.ort}`.trim(), data.kunde.land].filter(Boolean).join(', ')
    : null;
  ...
  if (data.iz)              notesParts.push(`IZ: ${data.iz}`);
  ...
  const result = db.prepare(`
    INSERT INTO orders (
      ...
      items_table, work_types, project_number, created_by
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?)
  `).run(
    orderNumber,
    'geplant',
...
async function processFile(filePath) {
  ...
  console.log(`[LS-Watcher] Verarbeite: ${filename}`);

  // Import-Eintrag anlegen
  const importResult = db.prepare(`
    INSERT INTO lieferschein_imports (filename, original_name, status)
    VALUES (?, ?, 'processing')
...
module.exports = { startWatcher, stopWatcher, getInboxDir, processFile };
