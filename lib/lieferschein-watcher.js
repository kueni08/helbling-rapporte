...
const { parseSwissAddress, formatSwissAddress, syncOrderAddressParts } = require('./address');
...
- "montage_adresse": vollständige Montageadresse / Einbauadresse, NUR wenn im Dokument klar ausgewiesen
- "montage_objekt", "montage_strasse", "montage_plz", "montage_ort": Montageadresse zusätzlich sauber in diese vier Bestandteile trennen
...
- "kontaktperson_vor_ort_telefon": Telefonnummer der Kontaktperson vor Ort, NUR wenn direkt bei der Kontaktperson genannt
...
Erwartetes JSON-Format:
{
  ...
  "montage_objekt": null,
  "montage_strasse": null,
  "montage_plz": null,
  "montage_ort": null,
  "kontaktperson_vor_ort": null,
  "kontaktperson_vor_ort_telefon": null,
  "kunde": {
    "name": "Sictech GmbH",
...
// Fallback für gescannte/gedruckte PDFs: sendet PDF direkt an Claude (Vision/PDF-API)
async function extractLieferscheinDataFromPdfBuffer(pdfBuffer) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    throw new Error('ANTHROPIC_API_KEY nicht konfiguriert. Bitte in .env eintragen.');
  }
  ...
  const db = getDb();
  const savedPromptTemplate = db.prepare('SELECT value FROM settings WHERE key = ?').get('extraction_prompt')?.value;
  // Prompt ohne {TEXT}-Platzhalter – der PDF-Inhalt wird als Dokument mitgesendet
  const promptTemplate = savedPromptTemplate || DEFAULT_EXTRACTION_PROMPT;
...
function createOrder(db, data, importId) {
  ...
  const parsedInstallation = parseSwissAddress(data.montage_adresse || '');
  const installationAddress = formatSwissAddress({
    name: data.montage_objekt || parsedInstallation.name,
    street: data.montage_strasse || parsedInstallation.street,
    postalCode: data.montage_plz || parsedInstallation.postalCode,
    city: data.montage_ort || parsedInstallation.city,
  }) || null;
  ...
  // Standard-Monteur aus Import-Einstellungen
  const defaultMonteurRow = db.prepare("SELECT value FROM settings WHERE key = 'default_import_monteur_id'").get();
  const assignedTo = defaultMonteurRow?.value ? parseInt(defaultMonteurRow.value) : null;

  const result = db.prepare(`
    INSERT INTO orders (
      order_number, status, customer_name, customer_address,
      installation_address, planned_date, notes_planer,
      items_table, work_types, project_number, orderer, on_site_contact, on_site_contact_phone,
  ...
  `).run(
    ...
    data.kontaktperson_vor_ort_telefon || null,
    assignedTo,
    null  // system-Import (kein user)
  );

  syncOrderAddressParts(db, result.lastInsertRowid, installationAddress);
...
async function processFile(filePath, originalName, driveFileId, options = {}) {
  const db = getDb();
  ...
  try {
    ...
    // Bei zu wenig Text (gescanntes/gedrucktes PDF): PDF direkt an Claude senden
    let data;
    if (!pdfText || pdfText.trim().length < 50) {
      console.log(`[LS-Watcher] Wenig Text gefunden (${pdfText?.trim().length || 0} Zeichen) – verwende PDF-Vision-Fallback`);
      data = await extractLieferscheinDataFromPdfBuffer(pdfBuffer);
    } else {
      data = await extractLieferscheinData(pdfText);
    }

    const duplicate = db.prepare(`
      SELECT li.id, li.lieferschein_nr, li.projekt_nr, li.order_id, o.order_number
      FROM lieferschein_imports li LEFT JOIN orders o ON o.id = li.order_id
      WHERE li.id != ? AND li.status = 'success' AND (
        li.file_hash = ? OR
        (? IS NOT NULL AND li.lieferschein_nr = ?) OR
        (? IS NOT NULL AND li.projekt_nr = ? AND COALESCE(li.kunde,'') = COALESCE(?, '') AND COALESCE(li.lieferdatum,'') = COALESCE(?, ''))
      ) LIMIT 1
    `).get(importId, fileHash, data.lieferschein_nr || null, data.lieferschein_nr || null,
      data.projekt_nr || null, data.projekt_nr || null, data.kunde?.name || null, data.lieferdatum || data.montagetermin || null);

    if (options.previewOnly) {
      db.prepare(`UPDATE lieferschein_imports SET status='pending', lieferschein_nr=?, kunde=?,
        projekt_nr=?, lieferdatum=?, raw_json=?, file_hash=?, processed_at=datetime('now') WHERE id=?`)
        .run(data.lieferschein_nr || null, data.kunde?.name || null, data.projekt_nr || null,
          data.lieferdatum || data.montagetermin || null, JSON.stringify(data), fileHash, importId);
      return { id: importId, data, duplicate, original_name: displayName };
    }
    if (duplicate) throw new Error(`Duplikat erkannt: bereits in Auftrag ${duplicate.order_number || duplicate.order_id}`);
    ...
    if (options.previewOnly) throw err;
  }
}
...
function normaliseEditedData(original, edited) {
  if (!edited || typeof edited !== 'object' || Array.isArray(edited)) return original;

  const result = {};
  for (const key of [
    'lieferschein_nr', 'montagetermin', 'bestell_nr', 'projekt_nr', 'iz', 'uz',
    'montage_adresse', 'montage_objekt', 'montage_strasse', 'montage_plz', 'montage_ort',
    'kontaktperson_vor_ort', 'kontaktperson_vor_ort_telefon',
  ]) {
    result[key] = textValue(Object.prototype.hasOwnProperty.call(edited, key) ? edited[key] : original?.[key]);
  }
  if (result.montagetermin && !/^\d{4}-\d{2}-\d{2}$/.test(result.montagetermin)) {
    throw new Error('Montagetermin muss im Format JJJJ-MM-TT angegeben werden');
  }

  const originalCustomer = original?.kunde || {};
  const editedCustomer = edited.kunde && typeof edited.kunde === 'object' ? edited.kunde : {};
  result.kunde = {};
  for (const key of ['name', 'strasse', 'plz', 'ort', 'land', 'kunden_nr', 'kontakt']) {
    result.kunde[key] = textValue(
      Object.prototype.hasOwnProperty.call(editedCustomer, key) ? editedCustomer[key] : originalCustomer[key]
    );
  }

  const sourceArticles = Array.isArray(edited.artikel) ? edited.artikel : (original?.artikel || []);
  if (sourceArticles.length > 100) throw new Error('Maximal 100 Artikelpositionen pro Import');
  result.artikel = sourceArticles.map(row => {
    const quantity = Number(row?.menge);
    return {
      artikel_nr: textValue(row?.artikel_nr, 100),
      beschreibung: textValue(row?.beschreibung, 1000),
      menge: Number.isFinite(quantity) && quantity > 0 ? quantity : 1,
      einheit: textValue(row?.einheit, 30) || 'Stk.',
      ist_schluessebox_montage: row?.ist_schluessebox_montage === true,
      durchmesser: textValue(row?.durchmesser, 50),
      ist_fremdfabrikat: row?.ist_fremdfabrikat === true,
    };
  }).filter(row => row.artikel_nr || row.beschreibung);

  return result;
}

function confirmImport(importId, allowDuplicate = false, editedData = null) {
  const db = getDb();
  const imp = db.prepare("SELECT * FROM lieferschein_imports WHERE id=? AND status='pending'").get(importId);
  if (!imp) throw new Error('Ausstehender Import nicht gefunden');
  const data = normaliseEditedData(JSON.parse(imp.raw_json || '{}'), editedData);
  db.prepare(`UPDATE lieferschein_imports SET lieferschein_nr=?, kunde=?, projekt_nr=?,
    lieferdatum=?, raw_json=? WHERE id=?`).run(
      data.lieferschein_nr || null,
      data.kunde?.name || null,
      data.projekt_nr || null,
      data.montagetermin || null,
  ...
  const duplicate = db.prepare(`SELECT li.*, o.order_number FROM lieferschein_imports li
    LEFT JOIN orders o ON o.id=li.order_id WHERE li.id != ? AND li.status='success' AND
    (li.file_hash=? OR (? IS NOT NULL AND li.lieferschein_nr=?) OR
    (? IS NOT NULL AND li.projekt_nr=? AND COALESCE(li.kunde,'')=COALESCE(?, '') AND COALESCE(li.lieferdatum,'')=COALESCE(?, ''))) LIMIT 1`)
    .get(importId, imp.file_hash,
      data.lieferschein_nr, data.lieferschein_nr,
      data.projekt_nr, data.projekt_nr,
      data.kunde?.name || null, data.montagetermin || null);
  if (duplicate && !allowDuplicate) throw new Error(`Duplikat erkannt: bereits in Auftrag ${duplicate.order_number || duplicate.order_id}`);
  const filePath = path.join(INBOX_DIR, imp.filename);
  const articlesImported = upsertArticles(db, data.artikel || []);
  const { orderId, orderNumber } = createOrder(db, data, importId);
  savePdfAsAttachment(db, orderId, orderNumber, filePath, imp.original_name);
  db.prepare(`UPDATE lieferschein_imports SET status='success', articles_imported=?, order_id=?, processed_at=datetime('now') WHERE id=?`)
    .run(articlesImported, orderId, importId);
  fs.renameSync(filePath, path.join(PROCESSED_DIR, imp.filename));
...
function startWatcher() {
  ...
  watcherInstance.on('add', async (filePath) => {
    ...
    if (path.basename(filePath).startsWith('pending_')) return;
