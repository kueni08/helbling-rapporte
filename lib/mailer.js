...
<h2>Zusätzliches Material (nicht auf LS)</h2>
<table>
  <tr><td style="background:#fff3cd;font-weight:bold">Artikel</td><td style="background:#fff3cd;font-weight:bold">Menge</td><td style="background:#fff3cd;font-weight:bold">Einheit</td></tr>
  ${order.extra_material.map(i => `<tr><td>${i.name||''}</td><td>${i.quantity||''}</td><td>${i.unit||''}</td></tr>`).join('')}
</table>` : '';
...
<table>
  ...
  ${order.items_table.map(i => {
    ...
    const name = (i.name || '').replace(/\s*–\s*.+$/, '').trim() || i.name || '';
    return `<tr style="${s}"><td>${name}</td><td>${i.quantity||''}</td><td>${i.unit||''}</td></tr>`;
  }).join('')}
</table>` : ''}
...
async function sendCompletionEmail(orderId) {
  ...
  let rapportAttachment = null;
  if (smtpConfigured) {
    try {
      const pdfBuffer = await generatePdf(rapportHtml);
      rapportAttachment = {
      ...
      console.warn('[Mailer] PDF fehlgeschlagen, sende HTML-Anhang:', pdfErr.message);
      rapportAttachment = {
        filename:    `Installationsrapport_${order.order_number || orderId}.html`,
        content:     Buffer.from(fullHtml, 'utf8'),
  ...
  if (smtpConfigured) {
    try {
      ...
      await transporter.sendMail({
        ...
        html:    fullHtml,
        attachments: [...mailAttachments, ...(rapportAttachment ? [rapportAttachment] : [])],
      });
  ...
  // Drive: HTML-Rapport speichern – läuft unabhängig von SMTP
  if (drive.isDriveEnabled()) {
    ...
    const dirRow = db.prepare('SELECT dir_name FROM order_attachments WHERE order_id = ? AND dir_name IS NOT NULL LIMIT 1').get(orderId)
      || db.prepare('SELECT dir_name FROM order_photos WHERE order_id = ? AND dir_name IS NOT NULL LIMIT 1').get(orderId);
    ...
    const dirName = dirRow?.dir_name || `${safeNum}_${dateStr}`;
...
module.exports = { getSmtpConfig, getTransporter, buildHtmlReport, generatePdf, sendCompletionEmail };
