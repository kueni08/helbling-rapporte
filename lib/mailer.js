...
function getSmtpConfig() {
  ...
  return {
    ...
    from: get('smtp_from') || '',
    completionTo: get('completion_email_to') || process.env.COMPLETION_EMAIL_TO || '',
    completionCc: get('completion_email_cc') || process.env.COMPLETION_EMAIL_CC || '',
    replyTo: get('completion_email_reply_to') || process.env.COMPLETION_EMAIL_REPLY_TO || '',
  };
}
...
async function sendCompletionEmail(orderId) {
  ...
  if (smtpConfigured && smtpCfg.completionTo) {
    try {
      const transporter = getTransporter();
      await transporter.sendMail({
        from:    `"Helbling Rapporte" <${smtpCfg.from || smtpCfg.user}>`,
        to:      smtpCfg.completionTo,
        cc:      smtpCfg.completionCc || undefined,
      ...
      console.log(`[Mailer] Abschluss-Mail gesendet: Auftrag ${order.order_number} → ${smtpCfg.completionTo}`);
    ...
    console.warn('[Mailer] SMTP oder Abschluss-Empfänger nicht konfiguriert – Abschluss-Mail wird nicht gesendet');
  }
...
async function sendPortalAccessEmail({ to, fullName, customerName, temporaryPassword }) {
  const cfg = getSmtpConfig();
  const useJsonTransport = process.env.NODE_ENV === 'test' && process.env.PORTAL_MAIL_TRANSPORT === 'json';
  if (!useJsonTransport && (!cfg.user || !cfg.pass)) {
    const error = new Error('SMTP ist nicht konfiguriert. Portalzugang wurde nicht erstellt.');
    error.code = 'SMTP_NOT_CONFIGURED';
    throw error;
  }
  const transporter = useJsonTransport ? nodemailer.createTransport({ jsonTransport: true }) : getTransporter();
  const portalUrl = process.env.PORTAL_BASE_URL || 'https://rapporte.helbling.net/kundenportal';
  const safeName = escapeHtml(fullName);
  const safeCustomer = escapeHtml(customerName);
  const safePassword = escapeHtml(temporaryPassword);
  await transporter.sendMail({
    from: `"Helbling Kundenportal" <${cfg.from || cfg.user || 'no-reply@example.test'}>`,
    to,
    subject: 'Ihr Zugang zum Helbling Kundenportal',
    text: `Guten Tag ${fullName}\n\nIhr Zugang zum Helbling Kundenportal für ${customerName} wurde eingerichtet.\n\nLogin: ${to}\nEinmalpasswort: ${temporaryPassword}\nPortal: ${portalUrl}\n\nBeim ersten Login müssen Sie ein eigenes Passwort setzen.\n`,
    html: `<p>Guten Tag ${safeName}</p><p>Ihr Zugang zum Helbling Kundenportal für <strong>${safeCustomer}</strong> wurde eingerichtet.</p>
      <p><strong>Login:</strong> ${escapeHtml(to)}<br><strong>Einmalpasswort:</strong> <code style="font-size:16px">${safePassword}</code></p>
      <p><a href="${escapeHtml(portalUrl)}">Helbling Kundenportal öffnen</a></p>
      <p>Beim ersten Login müssen Sie ein eigenes Passwort setzen.</p>`,
  });
  return { email_sent: true, email: to };
}

module.exports = { getSmtpConfig, getTransporter, buildHtmlReport, generatePdf, generatePdfSafe, sendCompletionEmail, sendPortalAccessEmail };
