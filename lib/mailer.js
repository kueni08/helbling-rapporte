...
async function sendCompletionEmail(orderId) {
  ...
  if (!smtpCfg.user || !smtpCfg.pass) {
    console.warn('[Mailer] SMTP nicht konfiguriert – Abschluss-Mail nicht gesendet');
    return;
  }
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to:      SIBOX_EMAIL,
      subject: `Montagerapport ${order.order_number} – ${order.customer_name || ''} – abgeschlossen`,
      html:    fullHtml,
  ...
  if (drive.isDriveEnabled()) {
    ...
    drive.uploadFile(tmpFile, `Rapport_${order.order_number || orderId}.html`, 'text/html', dirRow?.dir_name || null)
...
module.exports = { getSmtpConfig, getTransporter, buildHtmlReport, sendCompletionEmail };
