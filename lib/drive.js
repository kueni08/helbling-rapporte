...
function getDrive() {
  ...
  // Option A: OAuth2 mit persönlichem Google-Account (kein Workspace nötig)
  // Einrichtung: node scripts/setup-drive-oauth.js
  const oauthClientId     = process.env.GOOGLE_OAUTH_CLIENT_ID;
  const oauthClientSecret = process.env.GOOGLE_OAUTH_CLIENT_SECRET;
  const oauthRefreshToken = process.env.GOOGLE_OAUTH_REFRESH_TOKEN;
  if (oauthClientId && oauthClientSecret && oauthRefreshToken) {
    try {
      const oauth2 = new google.auth.OAuth2(oauthClientId, oauthClientSecret, 'urn:ietf:wg:oauth:2.0:oob');
      oauth2.setCredentials({ refresh_token: oauthRefreshToken });
      _drive = google.drive({ version: 'v3', auth: oauth2 });
      console.log('[Drive] OAuth2-Modus aktiv (persönliches Google-Konto)');
      return _drive;
    } catch (e) {
      console.error('[Drive] OAuth2 init error:', e.message);
      return null;
    }
  }

  // Option B: Service Account JSON als Umgebungsvariable (base64 oder direkt)
  ...
  // Option C: Service Account Dateipfad (für lokale Entwicklung)
  if (!credentials) {
...
async function uploadFile(filePath, displayName, mimeType, subfolderName) {
  ...
  try {
    ...
    const webViewLink = res.data.webViewLink || res.data.webContentLink || null;
    console.log('[Drive] Uploaded:', displayName, '->', fileId);
    return { id: fileId, webViewLink };
  } catch (e) {
    const details = e.response?.data?.error || e.message;
    console.error('[Drive] Upload error für', displayName, ':', typeof details === 'object' ? JSON.stringify(details) : details);
    return null;
  }
}
