...
function initDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS email_inbox (
      ...
      -- Verknüpfungen
      linked_inquiry_id INTEGER REFERENCES customer_inquiries(id),
      linked_order_id INTEGER REFERENCES orders(id),
      -- Metadaten
      raw_headers TEXT,
      error_message TEXT,
...
function runMigrations(db) {
  ...
  // Email-Agent attachment directory
  const emailAttDir = path.join(uploadsDir2, 'email-attachments');
  if (!fs.existsSync(emailAttDir)) fs.mkdirSync(emailAttDir, { recursive: true });

  // Email-Agent: neue Felder für Thread, Draft, Anhang-Texte, Quellordner
  try { db.exec("ALTER TABLE email_inbox ADD COLUMN ai_draft TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE email_inbox ADD COLUMN thread_json TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE email_inbox ADD COLUMN attachment_texts TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE email_inbox ADD COLUMN source_folder TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE email_attachments ADD COLUMN extracted_text TEXT"); } catch(e) {}
}
