...
function runMigrations(db) {
  ...
  for (const [column, definition] of Object.entries(userSecurityColumns)) {
    if (!userCols.includes(column)) db.exec(`ALTER TABLE users ADD COLUMN ${column} ${definition}`);
  }
  ...
  `);
  const duplicatePortalEmail = db.prepare(`SELECT lower(email) AS email FROM customer_portal_users
    GROUP BY lower(email) HAVING COUNT(*) > 1 LIMIT 1`).get();
  if (duplicatePortalEmail) {
    console.warn('[Migration] Doppelte Kundenportal-E-Mail verhindert eindeutigen Login:', duplicatePortalEmail.email);
  } else {
    db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_customer_portal_users_email ON customer_portal_users(email COLLATE NOCASE)');
  }
...
function seedDefaults(db) {
  ...
  if (!activeAdmin && process.env.NODE_ENV === 'production') {
    throw new Error('Produktivstart abgebrochen: In der Datenbank ist kein aktives Administratorkonto vorhanden.');
  }
  const defaultAdmin = db.prepare("SELECT id FROM users WHERE username = 'admin'").get();
  if (!defaultAdmin && process.env.NODE_ENV !== 'production') {
